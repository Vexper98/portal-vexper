...
import { Banknote, Plus, Search, CheckCircle2, RefreshCw, MessageCircle, Trash2 } from "lucide-react";
import UpgradeModal from "../components/contador/UpgradeModal";

const STATUS_COLORS = {
  pendente: "bg-amber-500/15 text-amber-400 border-amber-400/30",
  recebido: "bg-emerald-500/15 text-emerald-400 border-emerald-400/30",
  atrasado: "bg-red-500/15 text-red-400 border-red-400/30",
};
...
export default function ContasAReceber() {
  ...
  const [form, setForm] = useState({ company_id: "", descricao: "", referencia: "", valor: "", vencimento: "", status: "pendente", forma_recebimento: "", observacoes: "" });
  ...
  const load = async () => {
    ...
    const [comps, accs] = await Promise.all([
      ...
      base44.entities.AccountReceivable.filter({ owner_user_id: me.email }),
    ]);
    setCompanies(comps);
    setAccounts(accs);
    setLoading(false);
  ...
  const handleSave = async () => {
    ...
    await base44.entities.AccountReceivable.create({ ...form, valor: parseFloat(form.valor) || 0, owner_user_id: user.email, company_name: comp?.razao_social || "" });
    toast.success("Cobrança cadastrada!"); setSaving(false); setDialogOpen(false);
    setForm({ company_id: "", descricao: "", referencia: "", valor: "", vencimento: "", status: "pendente", forma_recebimento: "", observacoes: "" });
    load();
  };

  const handleReceive = async (id) => {
    await base44.entities.AccountReceivable.update(id, { status: "recebido", data_recebimento: new Date().toISOString().slice(0, 10) });
    toast.success("Marcada como recebida!");
    setAccounts(prev => prev.map(a => a.id === id ? { ...a, status: "recebido" } : a));
  };
  ...
  const handleWhatsApp = (acc) => {
    ...
    const msg = encodeURIComponent(`Olá ${comp?.razao_social}, passando para lembrar que há um valor pendente de R$ ${Number(acc.valor || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })} com vencimento em ${acc.vencimento || "—"}. Por favor, entre em contato.`);
    window.open(`https://wa.me/55${phone}?text=${msg}`, "_blank");
  };

  const handleDelete = async (id) => {
    await base44.entities.AccountReceivable.delete(id);
  ...
  if (!isPro(user)) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
      <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow-2xl shadow-green-500/40">
        <Banknote className="w-10 h-10 text-white" />
      </div>
      <div className="text-center">
        <h2 className="text-2xl font-black text-white mb-2">Contas a Receber</h2>
  ...
  const totalPendente = filtered.filter(a => a.status !== "recebido").reduce((s, a) => s + (Number(a.valor) || 0), 0);
  const totalRecebido = filtered.filter(a => a.status === "recebido").reduce((s, a) => s + (Number(a.valor) || 0), 0);
  const atrasados = filtered.filter(a => a.status === "atrasado").length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-white">Contas a Receber</h1>
            <p className="text-xs text-slate-500">{accounts.length} cobrança(s) · {atrasados} em atraso</p>
          </div>
        </div>
        <Button onClick={() => setDialogOpen(true)} className="bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0 hover:opacity-90 rounded-xl shadow-lg gap-2">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "A Receber", value: `R$ ${totalPendente.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, color: "from-amber-500 to-orange-500" },
          { label: "Recebido", value: `R$ ${totalRecebido.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, color: "from-green-500 to-emerald-600" },
          { label: "Em Atraso", value: atrasados, color: "from-red-500 to-rose-600" },
        ].map((s, i) => (
          ...
          {filtered.map((acc, i) => {
            ...
            return (
              <motion.div key={acc.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${acc.status === "recebido" ? "from-emerald-500 to-teal-600" : acc.status === "atrasado" ? "from-red-500 to-rose-600" : "from-amber-500 to-orange-500"} flex items-center justify-center`}>
                    <Banknote className="w-4 h-4 text-white" />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {acc.status !== "recebido" && (
                      <>
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-emerald-400 hover:bg-emerald-500/10 rounded-lg" onClick={() => handleReceive(acc.id)} title="Marcar recebido">
                          <CheckCircle2 className="w-3.5 h-3.5" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-green-400 hover:bg-green-500/10 rounded-lg" onClick={() => handleWhatsApp(acc)} title="Lembrete WhatsApp">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg rounded-3xl" style={{ background: "#0a1628", border: "1px solid rgba(34,197,94,0.2)", color: "white" }}>
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2"><Banknote className="w-5 h-5 text-green-400" /> Nova Cobrança</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            {[
              { field: "descricao", label: "Descrição *", placeholder: "Ex: Honorário mensal", colSpan: 2 },
              { field: "referencia", label: "Referência", placeholder: "Ex: Janeiro/2026" },
              { field: "valor", label: "Valor (R$) *", placeholder: "0,00", type: "number" },
              { field: "vencimento", label: "Vencimento", type: "date" },
              { field: "forma_recebimento", label: "Forma", placeholder: "PIX, Boleto..." },
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !form.company_id || !form.descricao}
              className="rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0 hover:opacity-90 gap-2">
